...
class Crop2x2Generator(CropGenerator):
    ...
    def __getitem__(self, index: int) -> tuple[np.ndarray, np.ndarray]:
        ...
        mask = np.empty(
            (
                self.batch_size,
                int(self.image_sizes[0] / 2),
                int(self.image_sizes[1] / 2),
            ),
            np.float32,
        )
        image = np.empty(
        ...
        for item in range(self.batch_size):
            ...
            image_file = tfk.preprocessing.image.load_img(
                self.datapath / self.df.iloc[index * self.batch_size + item]["ImageId"]
            )  # Uploading the image from the mask DataFrame
            image_array = (
                tfk.preprocessing.image.img_to_array(image_file) / 255
            )  # Conveting the image to an NP array
            image_width, image_height = image_file.size

            # Raise error if dataset shape is not homogenous
            if self.image_sizes != (image_width, image_height):
                raise ValueError(
                    f"Image {item} has wrong size. Should be {image_width}x{image_height}. Sample dataset should be homogenous."
                )
            ...
            image[item] = self.crop(
                image_array, i
            )  # Croping and retrieving part with maximum ship's area
        return image, mask
    ...
    def crop(self, img: np.ndarray, i: int) -> np.ndarray:
        ...
        """
        return img[
            (i // 2) * self.part_size : ((i // 2) + 1) * self.part_size,
            (i % 2) * self.part_size : (i % 2 + 1) * self.part_size,
        ]
...
class Crop3x3Generator(CropGenerator):
    ...
    def __getitem__(self, index: int) -> tuple[np.ndarray, np.ndarray]:
        ...
        mask = np.empty(
            (
                self.batch_size,
                int(self.image_sizes[0] / 3),
                int(self.image_sizes[1] / 3),
            ),
            np.float32,
        )
        image = np.empty(
    ...
    def crop(self, img: np.ndarray, i: int) -> np.ndarray:
        ...
        """
        return img[
            (i // 3) * self.part_size : ((i // 3) + 1) * self.part_size,
            (i % 3) * self.part_size : (i % 3 + 1) * self.part_size,
        ]  # Taking the required part of the original image
...
class NoCropGenerator(CropGenerator):
    ...
    def __getitem__(self, index: int) -> tuple[np.ndarray, np.ndarray]:
        # Creating empty NP arrays for image and mask
        mask = np.empty(
            (self.batch_size, self.image_sizes[0], self.image_sizes[1]), np.float32
        )
        image = np.empty(
            (self.batch_size, self.image_sizes[0], self.image_sizes[1], 3), np.float32
        )

        for item in range(self.batch_size):
            # Single batch processing
            image_file = tfk.preprocessing.image.load_img(
                self.datapath / self.df.iloc[item]["ImageId"]
            )  # Uploading the image from the mask DataFrame
            image_array = (
                tfk.preprocessing.image.img_to_array(image_file) / 255
            )  # Conveting the image to an NP array
            ...
            mask[item] = self.decode(
                self.df.iloc[index * self.batch_size + item][
                    "EncodedPixels"
                ],  # Retrieving, decoding the mask of the image and cropping part with maximum ship's area
                (image_width, image_height),
            )
