...
def downsample_block(input_tensor, filters):
    '''Convolution'''
    x = tfl.Conv2D(filters, kernel_size=3, activation='elu', padding='same', kernel_initializer='he_normal')(input_tensor)
    x = tfl.Conv2D(filters, kernel_size=3, activation='elu', padding='same', kernel_initializer='he_normal')(x)
    skip = x
...
def upsample_block(input_tensor, skip_tensor, filters):
    '''Deconvolution'''
    x = tfl.Conv2DTranspose(filters, 2, strides=(2, 2), padding='same')(input_tensor) #  // 2 at filters
    x = tfl.BatchNormalization()(x)
...
def unet_model(input_shape):
    ...
    # Output layer
    outputs = tfl.Conv2D(kernel_size=1, filters=1, activation='sigmoid', padding="same", kernel_initializer='he_normal')(up1)
