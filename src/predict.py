...
if __name__ == "__main__":
    ...
    first_image = tfk.preprocessing.image.load_img(
        DATASET_FOLDER / first_image_name
    )  # Loading first image
    ...
    for image in images:
        ...
        image_name = image
        image = tfk.preprocessing.image.load_img(
        ...
        if image.size != (image_width, image_height):
            raise ValueError(
                f"Image {image_name} has wrong size. Should be {image_width}x{image_height}. Sample dataset should be homogenous."
            )

        image_array: np.ndarray = (
            tfk.preprocessing.image.img_to_array(image) / 255
        ...
        if not masks:
            ...
            new_row = pd.DataFrame(
                {"ImageId": image_name, "EncodedPixels": np.nan}, index=[0]
            )
            ...
            for item in masks:
                ...
                new_row = pd.DataFrame(
                    {"ImageId": image_name, "EncodedPixels": item}, index=[0]
                )
                result_df = pd.concat([result_df, new_row], ignore_index=True)
