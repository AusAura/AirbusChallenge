...
class DataGenerator(ABC, tfk.utils.Sequence):
    ...
    def __init__(self, datapath: Path, batch_size: int, df_mask: pd.DataFrame):
        ...
        self.df =  df_mask.sample(frac=1) # DataFrame mask that allows to limit our dataset artificially
    ...
    def get_first_sizes(self) -> tuple[int, int]:
        ...
        '''
        first_image = tfk.preprocessing.image.load_img(self.datapath / self.df.iloc[0]['ImageId']) # Taking 1st image
