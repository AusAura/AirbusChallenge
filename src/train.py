...
# MASKS
EMPTY_MASK = train_set[train_set["EncodedPixels"].isna()].sample(
    EMPTY_IMAGES
)  # Random set of empty images
SHIPS_MASK = train_set[~train_set["EncodedPixels"].isna()].sample(
    SHIPS_IMAGES
)  # Random set of ship images
MASK_DF = pd.concat(
    [EMPTY_MASK, SHIPS_MASK]
)  # This mask could be used to artificially limit the dataset
...
def dice_score(y_true: tf.Tensor, y_pred: tf.Tensor) -> tf.Tensor:
    ...
    Returns tf.Tensor
    """
    return (2.0 * K.sum(y_pred * y_true) + 0.0001) / (
        K.sum(y_true) + K.sum(y_pred) + 0.0001
    )
...
if __name__ == "__main__":
    ...
    callback = tfk.callbacks.ModelCheckpoint(
        MODELS_FOLDER
        / "model.{epoch:02d}-val_loss-{val_loss:.4f}-dice-{dice_score:.4f}.keras",
        "val_loss",
        save_best_only=True,
    ...
    if loaded_history:
        loaded_history.history["loss"] = (
            loaded_history.history["loss"] + cur_history.history["loss"]
        )
        loaded_history.history["val_loss"] = (
            loaded_history.history["val_loss"] + cur_history.history["val_loss"]
        )
        loaded_history.history["dice_score"] = (
            loaded_history.history["dice_score"] + cur_history.history["dice_score"]
        )
        loaded_history.history["val_dice_score"] = (
            loaded_history.history["val_dice_score"]
            + cur_history.history["val_dice_score"]
        )
